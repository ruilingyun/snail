...
//        dd($roleId);
        ...
        if(!in_array($re,$roleId)){
            $messages =  "<script>alert('当前权限无法浏览此管理员列表页面!')</script>";
            return view('admin/index', compact('messages'));
        }
...
//            dd($r_id);
        ...
        // 获取普通用户角色名称
        $managername = DB::table('roles')
            ->where('id',$r_id)
            ->get();
        foreach ($managername as $v){
...
//        dd($users);
        return view('admin/managerlist', compact('users', 'display_name'));
    }
...
//        dd($userxq);
        ...
        return view('admin/managerdetail', compact('userxq'));
    }
